...
def identify_best_case(df):
    acceptable_cases = df[df['time'] <= 1.05 * df.loc[df['case'] == 1, 'time'].values[0]]
...
# calculate the targets for each combination of matrix_size and tile_size, which are the good cases
...
for matrix_size in train['matrix_size'].unique():
    for tile_size in train['tile_size'].unique():
        ...
        train.loc[(train['matrix_size'] == matrix_size) & (train['tile_size'] == tile_size) & (train_subset['total_energy'] < train_case1['total_energy'].values[0]) & (train_subset['time'] <= 1.05 * train_case1['time'].values[0]), 'target'] = 1
...
for matrix_size in test['matrix_size'].unique():
    for tile_size in test['tile_size'].unique():
        ...
        # set target to 1 for "good" cases
        test.loc[(test['matrix_size'] == matrix_size) & (test['tile_size'] == tile_size) & (test_subset['total_energy'] < test_case1['total_energy'].values[0]) & (test_subset['time'] <= 1.05 * test_case1['time'].values[0]), 'target'] = 1
        # set target to 2 for "best" case
...
for name, (model, params) in models.items():
    ...
    grid_search = GridSearchCV(pipeline, params, cv=cv, scoring='roc_auc_ovo')  # one-vs-one ROC AUC for multi-class problems
    ...
    for i in range(3):  # we have 3 classes: 0, 1, and 2
        print(f'Test AUC-ROC for {name}, class {i}: ', roc_auc_score(test_target_bin[:, i], test_pred_proba[:, i]))
    
    # predict best case for each combination of matrix_size and tile_size
    test['predicted_class'] = test_pred_class
    best_cases = test.loc[test.groupby(['matrix_size', 'tile_size'])['predicted_class'].idxmax()]
    columns_to_keep = ['matrix_size', 'tile_size', 'case', 'target', 'task1', 'task2', 'task3', 'task4', 'predicted_class']
